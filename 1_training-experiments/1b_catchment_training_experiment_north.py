...
Run MLP on all years of North catchment-scale data to predict a random year.
"""
...
from catchment_training_experiments_log import log_catchment_training_experiment, catchment_training_log_north
...
north_catchment_2000_2024_df = pd.read_csv(
    "./North_catchment_variables/north_catchment_2000_2024_vars.csv")
north_catchment_2000_2024_df['time'] = pd.to_datetime(
    north_catchment_2000_2024_df['time'], errors='coerce')
...
for i in range(100):
    ...
    withheld_mask = north_catchment_2000_2024_df['time'].dt.year == random_year
    test_random = north_catchment_2000_2024_df[withheld_mask].reset_index(
        drop=True)
    ...
    for N in range(1, len(remaining_years) + 1):
        ...
        train_mask = north_catchment_2000_2024_df['time'].dt.year.isin(
            train_years)
        train_df = north_catchment_2000_2024_df[train_mask].reset_index(drop=True)
    ...
    for N, df in training_sets.items():
        ...
        log_catchment_training_experiment(log_file=catchment_training_log_north,
