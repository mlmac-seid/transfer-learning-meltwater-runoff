...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 26 15:42:52 2026

@author: mlm211

Pre-train MLP with Minturn catchment-scale MAR predictors for 2000-2024.
"""
...
# Load 2000-2024 MAR data for Minturn
minturn_mar = pd.read_csv('./Minturn_catchment_variables/minturn_catchment_2000_2024_vars.csv')


# 2m air temperature, ice surface temperature, and albedo as predictors
t2m_ts_al2_swd = ['air_temp', 'ice_temp', 'albedo', 'shortwave_down']

# Fit scalers
x_scaler = StandardScaler().fit(minturn_mar[t2m_ts_al2_swd])
y_scaler = StandardScaler().fit(minturn_mar[["meltwater_runoff"]])

# # Scale training data
x_train = x_scaler.transform(minturn_mar[t2m_ts_al2_swd])
y_train_scaled = y_scaler.transform(
    minturn_mar[["meltwater_runoff"]]).flatten()
...
save_path_tl = './catchment_MAR_emulators/minturn_mar_mlp.keras'
...
# Save scalers
joblib.dump(
    x_scaler, './catchment_MAR_emulators/minturn_mar_xscaler.pkl')
joblib.dump(
    y_scaler, './catchment_MAR_emulators/minturn_mar_yscaler.pkl')
